...
-- Register widget
vicious.register(netwidget, vicious.widgets.net, '<span color="#CC9393">${wlo1 down_kb}</span> <span font=\"Terminus 8\">↓↑ </span> <span color="#7F9F7F">${wlo1 up_kb}</span>', 3)
...
for s = 1, screen.count() do
    ...
    -- Separators
    local sprt = wibox.widget.textbox(' || ')
    local leftBorder = wibox.widget.textbox(' [ ')
    local rightBorder = wibox.widget.textbox(' ]')
    ...
    right_layout:add(leftBorder)
    right_layout:add(batterywidget)
    right_layout:add(net_wired)
    right_layout:add(net_wireless)
    right_layout:add(sprt)
    right_layout:add(wibox.widget.textbox('Vol:'))
    right_layout:add(volume_widget)
    right_layout:add(sprt)
    right_layout:add(mytextclock)
    right_layout:add(rightBorder)
...
globalkeys = awful.util.table.join(
   awful.key({ modkey,           }, "Left",   awful.tag.viewprev       ),
    ...
    -- Pro ScreenLock 
    awful.key({ modkey,           }, "l", function () awful.util.spawn("lock") end), -- "lock" es un script definido en /bin/lock
    ...
    -- Cambio de teclado
    awful.key({ modkey,           }, "e", function() run_once("setxkbmap es") end),
    awful.key({ modkey,           }, "d", function() run_once("setxkbmap dvorak") end),
...
-- Change keyboard dist to dvorak y swap Caps y Ctrl
run_once("setxkbmap dvorak -option ctrl:swapcaps")
